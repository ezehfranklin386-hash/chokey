...
export default function SettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      ...
      {/* Tabs */}
      <div className="flex flex-wrap border-b border-primary-500/40 gap-1">
        {TABS.map((tab) => (
          <button
            key={tab.key}
...
function ProfileSection() {
  ...
  return (
    <div className="rounded-card border border-white-10 bg-primary-800 p-6 space-y-6">
      {/* Avatar */}
      <div className="flex items-center gap-4">
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gold-500/20 text-2xl font-bold text-gold-500">
          {profile?.fullName?.charAt(0) || profile?.email?.charAt(0) || '?'}
        </div>
        <div>
          <h2 className="text-sm font-bold text-white">{profile?.fullName || 'User'}</h2>
      ...
      <div className="h-px bg-primary-500/40" />

      {/* Form fields */}
      <div className="space-y-4 max-w-md">
        <div>
          <label className="text-xs text-white-50 mb-1 block">Email</label>
          <Input value={profile?.email || ''} readOnly className="text-white-50" />
        ...
        <div>
          ...
          <Input
            ...
            readOnly={!editing}
            className={!editing ? 'text-white-50' : ''}
          />
        </div>
        <div>
    ...
    <div className="rounded-card border border-white-10 bg-primary-800 p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white-50 mt-0.5">Add an extra layer of security to your account</p>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-white-50 mt-0.5">Change your account password</p>
          </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white-50 mt-0.5">Manage your active login sessions</p>
        </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white-50 mt-0.5">Create and manage API keys for programmatic trading</p>
        </div>
...
function NotificationsSection() {
  ...
  return (
    <div className="rounded-card border border-white-10 bg-primary-800 p-6 space-y-6">
      ...
      {/* Channels */}
      <div>
        <h3 className="text-xs font-medium text-white-70 uppercase mb-3">Delivery Channels</h3>
      ...
      {/* Events */}
      <div>
        <h3 className="text-xs font-medium text-white-70 uppercase mb-3">Events</h3>
        <div className="space-y-3">
...
function PreferencesSection() {
  ...
  return (
    <div className="rounded-card border border-white-10 bg-primary-800 p-6 space-y-6">
      ...
      <div className="space-y-4 max-w-md">
        <div>
          <label className="text-xs text-white-50 mb-1 block">Display Currency</label>
          <select
            value={currency}
            onChange={(e) => setCurrency(e.target.value)}
            className="w-full rounded-lg border border-primary-500 bg-primary-700 px-3 py-2.5 text-sm text-white focus:border-gold-500 focus:outline-none"
          >
        ...
        <div>
          <label className="text-xs text-white-50 mb-1 block">Default Slippage Tolerance</label>
          <select
            value={slippage}
        ...
        <div>
          <label className="text-xs text-white-50 mb-1 block">Default Trading View</label>
  ...
  return (
    <div className="rounded-card border border-white-10 bg-primary-800 p-6 space-y-6">
      ...
      <div className="space-y-4 text-sm">
        <div className="flex justify-between py-2 border-b border-primary-500/20">
          <span className="text-white-50">Version</span>
          <span className="text-white-90 font-mono">1.0.0</span>
        </div>
        <div className="flex justify-between py-2 border-b border-primary-500/20">
          <span className="text-white-50">Build</span>
          <span className="text-white-90 font-mono">2026.07.08</span>
        </div>
        <button className="flex w-full justify-between py-2 border-b border-primary-500/20 text-left hover:text-gold-500 transition-colors">
        ...
        <button className="flex w-full justify-between py-2 border-b border-primary-500/20 text-left hover:text-gold-500 transition-colors">
          <span className="text-white-50">Licenses</span>
          <svg className="h-4 w-4 text-white-30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <polyline points="9 18 15 12 9 6" />
          </svg>
        </button>
